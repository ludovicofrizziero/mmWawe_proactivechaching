...
T_tracking = 0.2; % tracking periodicity [s]     
v = 60/3.6; % speed [m/s]
...
rate_final = zeros(length(n_tx),n_rep_PL); %added by Frizziero
vector_lambda_bs = BS_per_km/1000; %added by Frizziero, rerunning the entire file with another value removes overhead, instead of a nested for. It allows also to split outputfiles accordingly more easily.
tic; %added by Frizziero
parfor antenna_idx = 1:length(n_tx) % consider single values of BS density
    %for vector_lambda_bs = BS_per_km/1000  %removed by Frizziero
        for n_iter = 1:n_rep_PL  
            ...
            for t = t_offset:t_offset:T_sim 
                ...
                space_offset = t * v; %t_offset * v; %modified by Frizziero
                ...
                end  %removed by Frizziero
                
                if mod(t, T_tracking) < 1e-10 %modified by Frizziero %% tracking is completed, select new beam pair --> moreover, all BSs possibly change their random orientations
            ...
            %fprintf('%10.6f s\n', toc(start));
            rate_final(antenna_idx, n_iter) = mean(rate); % need to save for all values of vector_lambda, antennas
        end % end n iterations
    %end % end vector_lambda %modified by Frizziero
end % end antennas

stop_timer = toc; %added by Frizziero
fprintf('runtime: %4.3f s\n', stop_timer);  %added by Frizziero
...
%end addition by Frizziero

fname = sprintf('rate_%dBSperKM.txt', BS_per_km);
fileID = fopen(fname,'w');
fprintf(fileID, formatSpec, tofile); %modified by Frizziero
fclose(fileID);